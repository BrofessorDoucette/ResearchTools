...
def interpolate_between_big_distances(
    x: Iterable, xp: Iterable, fp: Iterable, big_distances: Iterable = []
):
    ...
    if len(big_distances) > 0:
        ...
        for m, d in enumerate(big_distances):
            ...
            interpolated_chunk = np.interp(
                x, xp[start_index:end_index], fp[start_index:end_index], left=np.nan, right=np.nan
            )
        ...
        interpolated_chunk = np.interp(
            x, xp[start_index:], fp[start_index:], left=np.nan, right=np.nan
        )
        non_nan_values = np.isfinite(interpolated_chunk)
        interpolated_data[non_nan_values] = interpolated_chunk[non_nan_values]
