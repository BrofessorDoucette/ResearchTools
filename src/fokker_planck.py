...
def calculate_diffusion_coefficients(L, Kp, type: DIFFUSION_COEFFICIENT_TYPE_1D):
    ...
    match type:
        ...
        case DIFFUSION_COEFFICIENT_TYPE_1D.OZEKE:
            ...
            dD_ll_B_dL = (
                8 * a * (L**7) * exp_factor_B + a * (L**8) * np.log(10) * (2 * b * L + c) * exp_factor_B
            )
...
def fokker_planck_1D_simulation(
    sim_JD_grid,
    averaging_dt,
    sim_dt,
    dL,
    ...
    D_ll,
    dD_ll_dL,
    numerical_stability_scaling_factor=1e7,
):
    ...
    t_bin_edges = np.arange(
        start=sim_JD_grid[0], stop=sim_JD_grid[-1] + averaging_dt, step=averaging_dt
    )
    ...
    for T in range(len(min_BCs) - 1):
        ...
        df_dt = (dD_ll_dL[T, 1:-1] - (2.0 / initial_L[1:-1]) * D_ll[T, 1:-1]) * df_dL + D_ll[
            T, 1:-1
        ] * d2f_dL2
