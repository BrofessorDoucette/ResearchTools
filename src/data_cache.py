...
def psd_cache_exists(
    ...
    os_helper.verify_input_dir_exists(directory=cache_dir, hint="PSD CACHE DIR!")

    expected_file_name = psd_cache_file_name(
        satellite=satellite.upper(), start=start, end=end, mu=mu, k=k, field_model=field_model
    )
...
def cache_psd_at_selected_mu_and_k(
    refs: dict,
    satellite: str,
    start: datetime.datetime,
    end: datetime.datetime,
    ...
    os_helper.verify_output_dir_exists(
        directory=cache_dir, force_creation=make_dirs, hint="PSD CACHE DIR!"
    )

    output_file_name = psd_cache_file_name(
        satellite=satellite.upper(), start=start, end=end, mu=mu, k=k, field_model=field_model
    )
...
def load_psd_cache(
    ...
    input_file_name = psd_cache_file_name(
        satellite=satellite.upper(), start=start, end=end, mu=mu, k=k, field_model=field_model
    )
