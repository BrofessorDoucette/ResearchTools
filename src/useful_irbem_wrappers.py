...
def calculate_lstar_at_magnetic_equator_T89(dates, alt, lat, lon, kp):
    ...
    equator_model = IRBEM.MagFields(
        path=r"/project/spdr/opt/IRBEM/libirbem.so",
        options=[0, 0, 0, 0, 0],
        kext="T89",
        sysaxes=0,  # GDZ
        verbose=False,
    ...
    for T in range(len(dates)):
        ...
        locations_of_equator.append(
            equator_model.find_magequator(X=sat_coords, maginput=mag_inputs)["XGEO"]
        )

    locations_of_equator = np.vstack(locations_of_equator)

    lstar_model = IRBEM.MagFields(
        path=r"/project/spdr/opt/IRBEM/libirbem.so",
        options=[1, 0, 2, 2, 0],
        kext="T89",
        sysaxes=2,  # GSM!
    ...
    for T in range(0, len(locations_of_equator), 25):
        ...
        mag_inputs = {"Kp": kp[T : T + 25].flatten()}

        lstar_calculated.append(
            lstar_model.make_lstar(X=equator_coords, maginput=mag_inputs)["Lstar"]
        )
