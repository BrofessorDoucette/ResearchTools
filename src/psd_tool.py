...
def select_mu_and_k_from_psd(refs: dict, chosen_mu: float, chosen_k: float) -> dict:
    ...
    for T in range(len(JD)):
        ...
        try:
            ...
            K_coef = np.polynomial.polynomial.polyfit(
                x=(alpha_to_fit - alpha_of_min_k), y=k_to_fit, deg=4, full=False
            )
        ...
        E_coefs = np.array(
            [1, (2 * M_e), -1 * ((2 * M_e * B[T] * chosen_mu) / (np.sin(selected_alpha) ** 2))]
        )
        ...
        selected_L_star = accelerated_interp(
            selected_alpha, ALPHA[T, valid_Lstar], L_STAR[T, valid_Lstar]
        )
        ...
        for A in range(PSD.shape[1]):
            ...
            if np.any(valid_e):
                ...
                psd_per_alpha_at_selected_e.append(
                    np.exp(
                        accelerated_interp(
                            selected_E * 1000,
                            ENERGIES[T, valid_e] * 1000,
                            np.log(PSD[T, A, valid_e]),
                        )
                    )
                )
        ...
        extracted_PSD.append(
            accelerated_interp(
                selected_alpha, ALPHA[T, valid_a], np.asarray(psd_per_alpha_at_selected_e)[valid_a]
            )
        )
...
if __name__ == "__main__":
    ...
    start = datetime.datetime(year=2015, month=10, day=24)

    end = datetime.datetime(year=2015, month=11, day=1)

    t_total = time.time()

    dependencies = data_loader.load_psd(
        satellite="B", field_model=model.TS04D, start=start, end=end
    )
