...
def compress_month_rept_l2(
    ...
    os_helper.verify_output_dir_exists(
        directory=output_dir, force_creation=make_dirs, hint="COMPRESSED REPT DIR"
    )
    ...
    os_helper.verify_input_dir_exists(directory=input_dir, hint="RAW REPT DIR")
    ...
    while curr < end:
        ...
        rept_cdf_path_or_empty = glob.glob(
            pathname=f"rbsp{satellite.lower()}_rel03_ect-rept-sci-l2_{_year}{_month}{_day}*.cdf",
            root_dir=input_dir,
        )
    ...
    np.savez_compressed(
        output_file, FESA=fesa, L=L, MLT=mlt, EPOCH=epoch, ENERGIES=rept["FESA_Energy"][...]
    )
...
def calculate_and_compress_psd(
    satellite: str,
    field_model: model,
    month: int,
    year: int,
    ...
    os_helper.verify_output_dir_exists(
        directory=output_dir, force_creation=make_dirs, hint="PSD DEPENDENCY OUTPUT DIR"
    )

    if month < 10:
        output_file = os.path.join(
            output_dir, f"PSD_{year}0{month}_{satellite.upper()}_{field_model.name}.npz"
        )
    else:
        output_file = os.path.join(
            output_dir, f"PSD_{year}{month}_{satellite.upper()}_{field_model.name}.npz"
        )
    ...
    while curr < end:
        ...
        match field_model:
            case field_model.TS04D:
                magephem_h5 = (
                    f"rbsp{satellite.lower()}_def_MagEphem_TS04D_{_year}{_month}{_day}*.h5"
                )
        ...
        if len(magephem_h5_found) != 0:
            ...
            print(
                f"MAGEPHEM H5 NOT FOUND: {os.path.join(magephem_data_dir, magephem_h5)}. Skipping!"
            )
            curr += datetime.timedelta(days=1)
            continue

        emfisis_cdf = (
            f"rbsp-{satellite.lower()}_magnetometer_1sec-gse_emfisis-l3_{_year}{_month}{_day}*.cdf"
        )
        ...
        if len(emfisis_cdf_found) != 0:
            ...
            print(
                f"EMFISIS CDF NOT FOUND: {os.path.join(emfisis_data_dir, emfisis_cdf)}. Skipping!"
            )
            curr += datetime.timedelta(days=1)
            continue
        ...
        valid_energy_channels = (
            (0 < curr_ect_fedu_energy) & ((curr_ect_fedu_energy / 1000) < 10) & (curr_ect_fedu_energy_delta_plus > 0) & (curr_ect_fedu_energy_delta_minus > 0)
        )

        if not np.any(valid_energy_channels):

            print(
                f"All of the energy limits and energy channels were negative or zero, this file must be broken somehow! \nPath: {ect_cdf_path}. \nSkipping!"
            )
        ...
        curr_valid_ect_fedu_energy_delta_plus = curr_ect_fedu_energy_delta_plus[
            valid_energy_channels
        ]
        curr_valid_ect_fedu_energy_delta_minus = curr_ect_fedu_energy_delta_minus[
            valid_energy_channels
        ]

        curr_energy_maximums = (
            curr_valid_ect_fedu_energy + curr_valid_ect_fedu_energy_delta_plus
        ) / 1000.0
        curr_energy_minimums = (
            curr_valid_ect_fedu_energy - curr_valid_ect_fedu_energy_delta_minus
        ) / 1000.0
        ...
        curr_PSD[:, :, valid_energy_channels] = (
            (curr_ect_fedu[:, :, valid_energy_channels] / pc_squared) * 1.66e-10 * 200.3
        )  # CHEN 2005
        ...
        valid_B = (
            (curr_emfisis["magInvalid"][...] == 0) & (curr_emfisis["magFill"][...] == 0) & (curr_emfisis["calState"][...] == 0)
        )
        curr_emfisis_B = curr_emfisis_B[valid_B] / 100000.0  # Get B Field in Gauss
        curr_emfisis_JD = curr_emfisis_JD[valid_B]

        B = np.concatenate(
            (B, np.interp(curr_ect_JD, curr_emfisis_JD, curr_emfisis_B, left=np.NAN, right=np.NaN)),
            axis=0,
        )
        ...
        curr_magephem_alpha = np.deg2rad(
            np.concatenate(
                (
                    np.flip(curr_magephem_alpha, axis=0),
                    np.flip(curr_magephem_alpha, axis=0)[:-1] + 90,
                ),
                axis=0,
            )
        )  # We want alpha to go from 5 -> 90 -> 175 degrees
        curr_magephem_k = np.concatenate(
            (np.flip(curr_magephem_k, axis=1), curr_magephem_k[:, 1:]), axis=1
        )
        curr_magephem_Lstar = np.concatenate(
            (np.flip(curr_magephem_Lstar, axis=1), curr_magephem_Lstar[:, 1:]), axis=1
        )
        curr_magephem_L = np.concatenate(
            (np.flip(curr_magephem_L, axis=1), curr_magephem_L[:, 1:]), axis=1
        )

        K_interpolator = scipy.interpolate.RegularGridInterpolator(
            points=(curr_magephem_JD, curr_magephem_alpha),
            values=curr_magephem_k,
            bounds_error=False,
            fill_value=np.NaN,
        )

        Lstar_interpolator = scipy.interpolate.RegularGridInterpolator(
            points=(curr_magephem_JD, curr_magephem_alpha),
        ...
        L_interpolator = scipy.interpolate.RegularGridInterpolator(
            points=(curr_magephem_JD, curr_magephem_alpha),
