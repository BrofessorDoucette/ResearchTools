...
if __name__ == "__main__":
    ...
    MAGEPHEM = data_loader.load_raw_data_from_config(
        id=["RBSP", "MAGEPHEM", "T89D"],
        start=start_date,
        end=end_date,
        satellite=sat,
    ...
    dates = np.array(
        [
            datetime.datetime.isoformat(datetime.datetime.fromtimestamp(MAGEPHEM_TIME[t]))
            for t in range(len(MAGEPHEM_TIME))
        ]
    )
    ...
    np.savez(
        file=os.path.abspath(
            f"./../processed_data/chorus_neural_network/STAGE_1/Lstar/RBSP_{sat.upper()}_T89_{year}.npz"
        ),
