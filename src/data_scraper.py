...
def download_from_global_config(
    id: list[str],
    replace: dict = None,
    config_path: str = "",
    raw_data_dir: str = "",
    ...
    if "file_glob" not in id_config.keys():
        raise Exception(
            "Tried to download an ID with no file_glob set in the global config. I have no idea what the filename looks like or what type it is!"
        )

    if "url" not in id_config.keys():
        raise Exception(
            "Tried to download an ID with no url set in the global config. I have no idea what url to look for the file_glob within!!"
        )

    file_glob = global_configuration.replace_all_keys_in_string_with_values(
        id_config["file_glob"], map=replace
    )
    ...
    if "subdir" in id_config:
        subdir = global_configuration.replace_all_keys_in_string_with_values(
            id_config["subdir"], map=replace
        )
        output_dir = os.path.abspath(os.path.join(os.path.abspath(output_dir), *subdir.split("/")))

    os_helper.verify_output_dir_exists(
        directory=output_dir, force_creation=make_dirs, hint=f"OUTPUT DIR for: {id}"
    )

    if debug:
        print(f"OUTPUT DIR: {output_dir}")

    wget_r_directory(file_glob=file_glob, url=url, savdir=output_dir)
...
def download_year_omni_one_min_resolution(
    year: int,
    make_dirs: bool = False,
    config_path="",
    raw_data_dir: str = "",
    ...
    download_from_global_config(
        id=["OMNI", "ONE_MIN_RESOLUTION"],
        replace={"{$YEAR}": str(year), "{$MONTH}": ""},
...
def download_year_omni_one_hour_resolution(
    year: int,
    make_dirs: bool = False,
    config_path="",
    raw_data_dir: str = "",
...
def download_year_psd_dependencies(
    satellite: str,
    field_model: model,
    year: int,
    make_dirs: bool = False,
    ...
    download_from_global_config(
        id=["RBSP", "MAGEPHEM", field_model.name],
        replace={
            "{$SATELLITE}": satellite.lower(),
            "{$YEAR}": str(year),
            "{$MONTH}": "",
...
def download_month_rbsp_rept_l2(
    satellite: str, month: int, year: int, make_dirs: bool = False, config_path="", debug=False
) -> None:
...
def download_month_goes_eps(
    satid: str, month: int, year: int, make_dirs: bool = False, config_path="", debug=False
) -> None:
...
def download_year_poes_after_2012(
    satid: str, year: int, make_dirs: bool = False, config_path="", debug=False
) -> None:
...
def download_legacy_poes_1998_to_2014(
    satid: str, year: int, make_dirs: bool = False, config_path="", debug=False
) -> None:
...
if __name__ == "__main__":
    ...
    for year in range(2012, 2020):
        ...
        download_year_omni_one_min_resolution(
            year=year, make_dirs=True, raw_data_dir="./../raw_data/"
        )
